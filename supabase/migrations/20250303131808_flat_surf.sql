...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'wishlists_user_id_idx') THEN
    CREATE INDEX wishlists_user_id_idx ON wishlists(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'wishlists_creature_id_idx') THEN
